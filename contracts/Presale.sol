...
contract NaruPresale is Context, Ownable {
    ...
    function purchaseToken(uint256 amount) public payable nonReentrant {
        require(_msgSender() != address(0), "NaruPresale: AddressZero cannot purchase.");
        require(phaseIsActive(), "NaruPresale: Current phase is not active.");
        require(tokenPrice() != 0, "NaruPresale: Token Price is not set");
        require(amount * 1 ether <= _phaseSupplyLeft, "NaruPresale: Amount exceeds remaining supply of the current phase.");

        uint256 totalPrice = amount * _tokenPrice;
        require(msg.value >= totalPrice, "NaruPresale: Payment too low.");
    ...
    function setCurrentPhase(uint256 supply, uint256 timestampStart, uint256 timestampEnd) public onlyOwner {
        require(supply <= _token.balanceOf(address(this)), "NaruPresale: Supply value exceeds the token balance");
        require(timestampStart >= block.timestamp, "NaruPresale: opening time is before current time");
        require(timestampEnd > timestampStart, "NaruPresale: opening time is not before closing time");
